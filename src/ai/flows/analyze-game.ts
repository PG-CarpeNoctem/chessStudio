...
const AnalyzedMoveSchema = z.object({
  ...
  classification: z.enum(['Brilliant', 'Great', 'Best', 'Excellent', 'Good', 'Book', 'Inaccuracy', 'Mistake', 'Blunder', 'Forced', 'Critical', 'Okay', 'Theory']).describe("The classification of the move."),
...
2.  **analysis**: A detailed, move-by-move analysis. For each move in the PGN, provide its number, the player, the move in SAN, your classification, a concise explanation (1-2 sentences), and the centipawn evaluation of the position *after* the move. The evaluation should be from White's perspective (positive values favor White, negative values favor Black).
    ...
    - **Brilliant (!!)**: A very rare, difficult-to-find move that is not only the best but also involves a significant sacrifice or a deep, non-obvious strategic plan that secures a winning advantage.
    - **Critical (!)**: A difficult-to-find move that creates a significant advantage. This might not involve a sacrifice but is a key move that other strong moves do not match.
    - **Best (⭐)**: The top engine choice in the position that doesn't qualify as Brilliant or Critical but is clearly the optimal move.
    ...
    - **Okay**: A solid, developing move that is decent but not the best.
    - **Theory**: A standard, theoretical opening move.
...
4.  **moveCounts**: An object with 'white' and 'black' keys. For each player, provide a count of how many moves fell into each classification (Brilliant, Critical, Best, Excellent, Okay, Theory, Inaccuracy, Mistake, Blunder).
...
const analyzeGameFlow = ai.defineFlow(
  {
    ...
    // Map classifications before returning
    const {output} = await prompt(input);
    if (output) {
      output.analysis.forEach(move => {
        // Simple mapping for demonstration. Can be more complex.
        if ((move.classification as string) === 'Good') move.classification = 'Okay';
        if ((move.classification as string) === 'Book') move.classification = 'Theory';
        if ((move.classification as string) === 'Great') move.classification = 'Critical';
      });

      const mapCounts = (counts: any) => {
        const newCounts: any = {};
        for (const key in counts) {
          if (key === 'good') newCounts['okay'] = counts[key];
          else if (key === 'book') newCounts['theory'] = counts[key];
          else if (key === 'great') newCounts['critical'] = counts[key];
          else newCounts[key] = counts[key];
        }
        return newCounts;
      }
      if (output.moveCounts) {
          output.moveCounts.white = mapCounts(output.moveCounts.white);
          output.moveCounts.black = mapCounts(output.moveCounts.black);
      }
    }
    return output!;
  }
